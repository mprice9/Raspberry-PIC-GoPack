...
extern unsigned int ain0Buff[SAMP_BUFF_SIZE];
...
void init_UART(void){
    ...
    U1MODEbits.BRGH = 0;   //low speed baud (enables 16x RX sampling vs 4x with high speed mode)
...
/*
// Resets the transmit buffer to the first register spot after a complete transmission
...
// Flag goes high when a message is received.
...
*/
//////////////////////////////////////////////////////////////////////////////
// PWM Initialization (Motor control)
//////////////////////////////////////////////////////////////////////////////
...
void init_spi1(void)
{
	...
	SPI1CON1bits.CKE = 0; 		// Serial output data changes on transition from idle clock 
...
//	SPI1CON1bits.PPRE= 0b01;	//primary prescale is 16:1
	...
	SPI1STATbits.SPIROV = 0; 		// No recieve overflow has occurred
	SPI1STATbits.SPIEN = 1; 	// Enable SPI module

	// SPI1 Interrupt Controller Settings
	IFS0bits.SPI1IF = 0; // Clear the Interrupt flag
...
void init_dma0_spi1_tx(void)
{
	...
	DMA0CONbits.MODE  = 2;
	DMA0CONbits.DIR   = 1;
	DMA0CONbits.SIZE  = 0; // One-shot, Post-Inc., RAM-to-peripheral
	DMACS0 = 0;
...
void init_spi2(void)
{
   //SPI2 pins - Master mode (peripherals are slaves)
    RPOR3bits.RP7R = 10;       // Assign SDO2 to RP7, Pin 43 - OUTPUT
    RPOR2bits.RP5R = 11;     // Assign SCK2 to RP5, Pin 41 - OUTPUT
    RPINR22bits.SDI2R = 6;     // Assign SDI2 to RP6, Pin 42 - INPUT 
	...
	SPI2CON1bits.CKE = 0; 		// Serial output data changes on transition from idle clock 
								// state to active clock state
	SPI2CON1bits.CKP = 0; 		// Idle state for clock is a low-level; active state is a high-level
	//To get 1Mhz = 64Mhz/2/2/16
	SPI2CON1bits.SPRE= 0b110;	//secondary prescale is 2:1
	SPI2CON1bits.PPRE= 0b01;	//primary prescale is 16:1
	SPI2CON1bits.MSTEN = 1; 	// Master mode ensabled
	SPI2STATbits.SPIROV=0; 		// No recieve overflow has occurred
...
void init_encoder1(void)
{
    ...
    QEI1CONbits.QEIM = 6;       //Set to 6 if using index pulse
...
//    AD1CON2bits.SMPI = 1; // Select 1 conversions between interrupt
...
///*
// * Initialize SPI connection with Hall effect sensors, write sensor settings
// * 
// * Inputs:
// *      int module - the sensor to be used, either 1 (force) or 2 (steering angle)
// */
//
//void init_spi_connection(int module)
//{
//    MemoryRead_msg(module);
//    __delay_ms(10);
//    MemoryRead_msg(module);
//    __delay_ms(10);
//    NOP_COMMAND(module);
//    __delay_ms(10);
//    EEpromWrite_Message(module);
//    __delay_ms(10);
//    EERead_Challenge(module);
//    __delay_ms(10);
//    EEChallenge_Ans(module);
//    __delay_ms(40);
//    NOP_COMMAND(module);
//    __delay_ms(10);
//    Reboot(module);
